...
//
// 参数分级：
//   - --help      只展示 P0 常用参数（15 个）
//   - --help-all  展示全部参数（含 P1/P2 高级参数）
//
// 实现方式：P1/P2 flag 使用 MarkHidden 隐藏，--help-all 时手动遍历全部 flag 输出。
...
常用参数使用 --help 查看，高级参数使用 --help-all 查看全部。`,
		Example: `  # 基本扫描（使用内置字典）
  ...
  neoagent scan dir --targets-file targets.txt`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			// --help-all：输出全部 flag（含 hidden），然后退出
			if helpAll {
				printAllFlags(cmd)
				return nil
			}
	...
	flags.MarkHidden("network-interface")

	// --help-all flag 本身也隐藏
	flags.BoolVar(&helpAll, "help-all", false, "展示全部参数（含高级参数）")
	flags.MarkHidden("help-all")

	return cmd
}

// printAllFlags 打印 cmd 的全部 flag（包括 hidden），用于 --help-all。
// 通过临时重置 Hidden 字段后调用 FlagUsages() 实现。
func printAllFlags(cmd *cobra.Command) {
	cmd.Println(cmd.Long)
	cmd.Println()
	cmd.Println("Usage:")
	cmd.Printf("  %s [target] [flags]\n", cmd.CommandPath())
	cmd.Println()
	cmd.Println("Flags:")

	// 临时重置所有 flag 的 Hidden 字段，输出后恢复
	cmd.Flags().VisitAll(func(f *pflag.Flag) {
		f.Hidden = false
	})
	defer cmd.Flags().VisitAll(func(f *pflag.Flag) {
		// 恢复 help-all 自身的 hidden 状态
		if f.Name == "help-all" {
			f.Hidden = true
		}
	})
	...
	cmd.Println()
	cmd.Println("使用 neoagent scan dir --help 查看常用参数。")
}
