...
- [ ] **5.1.1 Web 扫描 CDN 边缘节点识别**（Phase 5.1 之后的独立小任务，不与 5.2 混排）：对 CDN 边缘节点做全套 Web 扫描（截图/BFS 深度爬取）纯属浪费资源且产出误报结果，方案见 [`docs/爬虫/Web扫描CDN识别方案.md`](./爬虫/Web扫描CDN识别方案.md)，实施步骤见 [`docs/爬虫/Web扫描CDN识别实施文档.md`](./爬虫/Web扫描CDN识别实施文档.md)。**状态**: 🏃 进行中（6/7 步）。
