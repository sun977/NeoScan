...
## 6. 未来演进 (Pipeline Concurrency)
在实现 `scan run` (全流程编排) 后，并发模型需要支持**流水线并发**：
- **Stage 1 (Alive)** 完成后，立即将存活 IP 喂给 **Stage 2 (Port)**，而不是等待所有 IP 扫完。
- 使用 `go channel` 连接各个 Stage 的 Runner，实现流式处理。
