...
现在进入 **Phase 3: 原生能力实现 (Execution Phase)**。

*   **Architecture (架构)**: **90%**。Cobra 命令树、Options 模式、Core Task 模型已完成，Reporter 接口已定义。
*   **Core Capability (核心能力)**: **60%**。IpAliveScanner 已实现 (RTT/TTL/OS/Hostname)，PortServiceScanner 已实现 (Nmap 引擎 + 自定义探针)，Console Reporter 支持表格输出。
*   **Infrastructure (基础设施)**: **70%**。Nmap 规则库静态打包完成，网络库 (Dialer/Proxy) 重构完成。
...
| **Reporter** | 🟢 **Done** | `ConsoleReporter` 已实现，支持 Tabular 输出。 |
...
| **Output Config** | 🟢 **Done** | 全局支持 CSV/JSON 输出 (`--oc`, `--oj`)。 |

### 1.3 Native Capabilities (原生能力) - "The Power"
| 组件 | 状态 | 备注 |
| :--- | :--- | :--- |
| **Concurrency** | 🟡 **Partial** | `IpAliveScanner` 中实现了基础的 Semaphore，需推广到通用 Runner。 |
| **Host Discovery** | 🟢 **Done** | `IpAliveScanner` 已实现 ARP/ICMP/TCP 探测，支持 OS/Hostname/RTT/TTL 解析。 |
| **Port Scan** | 🟢 **Done** | 原生 TCP Connect Scan，支持并发控制。 |
| **Service Scan** | 🟢 **Done** | `NmapEngine` 移植完成，支持官方/自定义规则混合加载。 |
| **Web Finger** | 🔴 **Pending** | 基于 `httpx` 或原生 HTTP 库的指纹识别。 |
...
### P0: Phase 3.6 - 高级并发优化
*   **目标**: 解决 TCP Connect Scan 的性能瓶颈。
*   **行动**:
    1.  优化 `Runner` 接口，支持动态并发调整。
    2.  实现 SYN Scan (Raw Socket) (可选，视需求而定)。

### P1: Phase 4.1 - 全流程编排 (Scan Run)
*   **目标**: 实现 `neoAgent scan run`。
*   **行动**:
    1.  实现 `internal/core/pipeline` 框架。
    2.  实现 `cmd/agent/scan/run.go` CLI 入口。
    3.  串联 Alive -> Port -> Service 流程。

### P2: Phase 4.2 - 集群适配 (Cluster Adapter)
...
*   **风险**: 外部工具依赖。Masscan/Nmap/Nuclei 的调用封装可能比较复杂，需要处理跨平台路径和权限问题。
*   **阻碍**: 无。
...
CLI 骨架和核心数据结构已经非常漂亮。
我们已经有了枪（CLI），有了子弹规格（Task Model），现在需要造**撞针（Runner）**。
立即启动 **Phase 3: 原生能力建设**。
