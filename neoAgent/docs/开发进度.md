...
*   **行动**:
    1.  **Payload DTO**: 🟢 **Done**。创建 `internal/model/adapter`，定义符合数据契约的 DTO。
    2.  **Adapter**: 🟢 **Done**。完善 `internal/service/adapter`，实现 `ToCoreTask` 和 `ToMasterResult`。
    3.  **Communication**: 🟢 **Done**。完善 `internal/service/client`，实现 HTTP 通信逻辑。
