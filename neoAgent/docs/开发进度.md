...
### [2026-02-04] Brute Scanner 增强
**协议栈完善**:
1.  **RDP Native**: 移植并净化了 `sbscan/grdp` 协议栈，移除了图形化/外设代码，实现了纯 Go 的 NLA (CredSSP/NTLMv2) 和 Standard RDP 认证。
2.  **PostgreSQL**: 基于 `lib/pq` 实现了 Postgres 认证，支持 `sslmode=disable` 和连接超时控制。
3.  **全协议覆盖**: 完成了 FTP, MongoDB, ClickHouse, SMB, MSSQL, Oracle, Telnet, Elasticsearch, SNMP 等协议的支持。

**CLI 体验优化**:
1.  **多端口支持**: 统一了 `utils.ParseIntList` 解析器，现在 Brute 和 PortService 均支持 `-p 80,443,1000-2000` 格式。
2.  **全量模式**: 引入 `--all (-a)` 参数，允许用户在找到第一个密码后继续尝试剩余字典。
3.  **输出优化**: 实现了 `TabularData` 接口，爆破结果以高亮表格展示；拒绝刷屏，默认仅显示结果，Debug 模式显示过程。
