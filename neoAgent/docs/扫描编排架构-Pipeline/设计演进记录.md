...
## 2026-02-04: 全流程编排升级 —— 从 "Linear Pipeline" 到 "Parallel Dispatch"

### 1. 演进背景 (Context)

**原始设计 (v1.0)**:
线性执行流程：`Alive -> Port -> Service -> Vuln/Brute`。
- 优点：逻辑简单，依赖清晰。
- 缺点：
    1.  **Head-of-Line Blocking**: 慢速任务（如 Web 爬虫）会阻塞后续不相关的任务（如 MySQL 爆破）。
    2.  **Resource Inefficiency**: 无法充分利用多核 CPU 和网络带宽，因为同一时间只有一种类型的 Scanner 在运行。

**新需求 (v2.0)**:
需要支持更复杂的调度策略，以提高扫描效率并降低被封禁风险。

### 2. 核心审查 (Linus Philosophy Check)

#### 2.1 Good Taste (品味)
- **现状**: 用一个巨大的 `PipelineRunner` 串行调度所有任务，逻辑僵化。
- **改进**: 引入 `ServiceDispatcher` (分发器) 和 `WorkerPool` (工作池)。Phase 1 (基础探测) 保持串行以确保数据质量，Phase 2 (攻击面评估) 并行化以提升效率。
- **结论**: 通过解耦 "探测" 和 "利用" 阶段，系统结构更符合实际业务流。

#### 2.2 Pragmatism (实用主义)
- **解决的问题**: 
    1.  **速度**: 并行扫描显著减少整体耗时。
    2.  **风控**: 实现了 `Vuln First, Brute Last` 策略，避免在漏洞扫描完成前进行高噪音的爆破，减少被 IDS 阻断的概率。

#### 2.3 Never Break Userspace (向后兼容)
- **约束**: CLI 的 `scan run` 命令行为需保持一致。
- **方案**: 内部调度逻辑升级，对外输出格式和参数保持不变。

### 3. 架构演进方案 (Architecture Evolution)

#### 3.1 核心变更
引入 **Phase-based Orchestration**:

- **Phase 1 (Sequential)**: 
  - `IpAliveScanner` -> `PortServiceScanner`
  - 目标：快速获取准确的资产清单 (IP, Port, Service, Banner)。

- **Phase 2 (Parallel Dispatch)**:
  - `ServiceDispatcher`: 接收 Phase 1 的结果，按协议分发任务。
  - **Queues**:
    - `WebQueue`: HTTP/HTTPS 服务 -> WebScanner
    - `VulnQueue`: 匹配到 CVE/POC -> NucleiScanner (High Priority)
    - `BruteQueue`: 支持的协议 -> BruteScanner (Low Priority)
  - **Dependencies**:
    - Brute 任务在同端口的 Vuln 任务完成后才开始执行 (Signal Notification)。


---

## 2026-02-05: 编排设计融合 (Fusion Design) —— 硬编码智能与数据流复用

### 1. 背景：新旧方案的冲突
- **旧方案 (Pipeline 后期升级规划)**: 强调 `Preset` 模板和 `YAML` 配置，设计了优秀的 `PipelineContext` 数据模型，但采用了低效的线性执行流。
- **新方案 (Parallel Dispatch)**: 强调并行分发和代码逻辑驱动，执行效率高，但数据流设计尚未固化。

### 2. 融合决策 (The Best of Both Worlds)
我们决定融合两者的优点，摒弃过度设计。

- **保留 (Keep)**: 
    - **PipelineContext**: 旧方案的数据模型非常清晰，直接作为贯穿 Phase 1 和 Phase 2 的核心载体。
    - **Preset 概念**: 保留 `Preset` 作为策略配置的入口（如 `Quick`, `Full`），但不再作为 YAML 模板加载器。
    - **Parallel Dispatch**: 采用新方案的架构，使用 `ServiceDispatcher` 进行任务分发。

- **摒弃 (Drop)**:
    - **External YAML**: 砍掉 YAML 解析和外部模板加载。Agent 的行为应由代码逻辑（Dispatcher）控制，而非用户编写的复杂配置文件。
    - **Linear Flow**: 彻底放弃线性流程，拥抱 Phase-based 并行流程。

### 3. 最终架构
- **Phase 1 (Sequential)**: `AutoRunner` 执行 `Alive -> Port -> Service`，填充 `PipelineContext`。
- **Handoff**: `AutoRunner` 将 Context 传递给 `ServiceDispatcher`。
- **Phase 2 (Parallel)**: `ServiceDispatcher` 根据 `PipelineContext.Services` 和 `Preset Strategy`，将任务分发到 `WebWorker`, `VulnWorker`, `BruteWorker`。
...
## 2026-02-04: 全流程编排并行化与分层策略

### 1. 背景：线性流程的瓶颈
早期的 Pipeline 设计采用简单的线性串联 (`Alive -> Port -> Web -> Brute`)。
**问题**:
- **性能短板**: Web 扫描通常很慢，阻塞了对其他端口（如 3306, 6379）的后续处理。
- **资源浪费**: 当 CPU 等待 Web 响应时，其他高吞吐的扫描模块（如 Brute）处于闲置状态。
- **风控风险**: 如果不加区分地对所有服务同时发起高频请求（Nuclei + Brute），极易触发防火墙。

### 2. 核心决策：服务驱动的分发模式 (Service-Driven Dispatching)
**变更**: 从 "Linear Pipeline" 演进为 "Parallel Dispatch Pipeline"。

**逻辑**:
1.  **Phase 1 (Sequential)**: 基础信息收集 (Alive -> Port -> Service)，必须串行以确保数据准确。
2.  **Phase 2 (Parallel)**: 基于 Phase 1 的结果，通过 **Dispatcher** 将任务分发到不同的并行队列。
    - **Web Queue**: 处理 HTTP/HTTPS 服务。
    - **Vuln Queue**: 处理高危漏洞检测 (Nuclei)。
    - **Brute Queue**: 处理弱口令爆破。

### 3. 优先级与依赖策略 (Vuln First, Brute Last)
为了解决风控和价值问题，确立了严格的优先级：
- **High Priority (Vuln)**: 优先执行漏洞扫描。如果直接发现 RCE，无需再进行爆破。且漏扫流量较小，隐蔽性好。
- **Low Priority (Brute)**: 爆破作为最后的手段。且必须在同端口的漏洞扫描完成后执行（虽然不同端口可并行），以防止因爆破触发封禁而导致漏扫。

### 4. 收益
- **最大化吞吐**: 充分利用带宽和连接池，互不阻塞。
- **安全性提升**: 通过优先级控制降低了被封禁的风险。
- **架构解耦**: Dispatcher 模式使得新增扫描模块（如 POC 验证）变得非常容易，只需增加一个分发规则。
