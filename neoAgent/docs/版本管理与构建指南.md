...
| 日期 | 版本 | 变更说明 |
|------|------|---------|
| 2026-08-14 | v1.1 | 新增 Tools 辅助工具说明 |
...
```
neoAgent/tools/
├── merge2main.ps1    # Windows PowerShell 分支合并脚本
├── merge2main.sh     # Linux/macOS Bash 分支合并脚本
└── 工具.txt           # 工具清单（待补充）
```

### merge2main — Dev 到 Main 分支合并工具

**功能**：将当前 dev 分支一键合并到 main 分支并推送到远程仓库，自动处理分支切换和错误回滚。

**适用场景**：开发完成阶段性功能后，从 dev 分支合入 main 分支。
...
**安全保障**：
- 步骤 2 合并失败时：自动切回原分支，不残留合并状态
- 步骤 3 推送失败时：自动 `git reset --hard` 回滚，切回原分支
- 非 dev 分支运行时：先警告，需用户确认后才继续

**注意事项**：
- 脚本会检查当前是否在 dev 分支上（分支名以 `dev` 开头）
- 合并前请确保当前工作区干净（无未提交的更改）
- 合并失败需要手动解决冲突
