...
| 日期 | 版本 | 变更说明 |
|------|------|---------|
| 2026-08-14 | v1.1 | 标注过期内容，补充 WebScanner / API Scanner 已落地信息 |

---

## 1. 背景与挑战

随着 NeoScan Agent 的能力不断扩展（从 Alive, Port, OS, Brute 到 WebScan, API Scanner），我们在代码架构上面临着严峻的挑战。

### 1.1 核心矛盾
我们拥有三种截然不同的使用场景，但它们依赖于同一套核心扫描能力：
1.  **Orchestration (编排模式)**: `scan run`。一键自动化，追求速度与覆盖率的平衡，需要智能决策和流程编排。
2.  **Atomic (原子模式)**: `scan brute/port/dir/api`。单点工具，追求深度与可控性，完全听从用户指令。
...
graph TD
    ...
    subgraph "Module Layer (模块层 - 原子能力)"
        ...
        R -->|Instance| M4[Brute Scanner]
        R -->|Instance| M5[Web Scanner]
        R -->|Instance| M6[API Scanner]
        R -->|Instance| M7[Dir Scanner]
    end
```

> **说明**: 原图仅包含 Alive/Port/OS/Brute/Web(未来)，实际代码中 Web、API、Dir 扫描器工厂均已落地。
...
### 3.2 阶段二：建立全局工厂 (已完成)
*   **动作**: 创建 `internal/core/factory/` 下的工厂文件，将扫描器的构建逻辑集中管理。
*   **收益**: `NewAutoRunner` 和 `NewRunnerManager` 代码大幅简化，逻辑更清晰。
*   **实际落地**: 目前有 6 个工厂文件：`alive_factory.go`, `port_factory.go`, `os_factory.go`, `brute_factory.go`, `web_factory.go`, `api_factory.go`

### 3.3 阶段三：集群能力动态上报 (未验证)
*   **动作**: 修改 Server 的心跳逻辑，通过 Factory 获取当前支持的 Scanner 列表并上报。
*   **收益**: Master 可以根据 Worker 的实际能力进行智能调度（例如：只把 Web 扫描任务发给安装了 Web 插件的 Worker）。
*   **状态**: ⚠️ 文档声称"Future"，经核查代码未发现心跳动态上报能力的实现，可能尚未落地。

## 5. 当前 Factory 全景

| 工厂文件 | 扫描器类型 | 状态 | 说明 |
|---------|-----------|:---:|------|
| `alive_factory.go` | `IpAliveScanner` | ✅ | 单机 + 集群双路径 |
| `port_factory.go` | `PortServiceScanner` | ✅ | 包含端口发现 + 服务识别 |
| `os_factory.go` | `OSScanner` | ✅ | 基于 TTL + Banner + Nmap 指纹 |
| `brute_factory.go` | `BruteScanner` | ✅ | 全功能版，15+ 协议注册 |
| `web_factory.go` | `WebScanner` | ✅ | 含浏览器管理器、自适应限流、指纹引擎 |
| `api_factory.go` | `ApiScanner` | ✅ | JS 接口提取，BFS 爬取 |
| ~~`dir_factory.go`~~ | ~~DirScanner~~ | 📝 | **待创建**（DirScanner 仅 CLI 独立命令，未接入 Factory） |
...
✅ **推荐立即执行**:
该方案是解决当前代码 "坏味道" 的最佳实践，且风险极低。它为 Phase 4.1 的并行编排和未来的扫描能力扩展铺平了道路。
