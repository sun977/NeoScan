...
`PipelineRunner` 将负责串联各个 Scanner：

1.  **Stage 1: Host Discovery**
    - Input: CIDR/IPs
    - Action: 调用 `IpAliveScanner`
    - Output: Alive IPs List
    - *Decision*: 如果无存活，终止流程。

2.  **Stage 2: Port Scanning**
    - Input: Alive IPs
    - Action: 调用 `PortServiceScanner` (Rate Limited)
    - Output: Open Ports List (Struct: IP:Port:Service)

3.  **Stage 3: Service Enumeration (Parallel)**
    - **Branch A: Web Scan**
        - Condition: Service == http/https
        - Action: 调用 `WebScanner`
    - **Branch B: Brute Force**
        - Condition: Service in [ssh, mysql, redis...] && `--burst` enabled
        - Action: 调用 `BruteForceScanner`

4.  **Stage 4: Vulnerability Scanning**
    - Input: Service Details + Web Fingerprints
    - Action: 调用 `VulnScanner` (Matching Tags)
