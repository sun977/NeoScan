...
> ⚠️ **本方案已废弃并回退（2026-08-07）**：本方案提出的"`crawler` 下沉为 `core/lib/crawler` 通用模块，由 `WebScanner`/`ApiScanner` 共享调用"的架构已被推翻。最终决策是：**每个原子扫描器功能自成一体、互不依赖，各自实现自己的抓取/分析能力，不设立跨扫描器共享的爬虫模块**。`crawler` 包已迁回 `internal/core/scanner/web/crawler`，重新变为 `WebScanner` 的私有内部实现；`core/lib/crawler/fetch.go`（`FetchAndCrawl` 统一入口）已删除。`ApiScanner` 保留骨架（`Name()`/`Run()`），但 `Run()` 不再调用任何共享抓取能力，直接返回空结果，待后续独立实现。本文档以下内容仅作历史决策记录保留，**不代表当前代码状态，不要按本文档的方案设计继续开发**。新增原子扫描器请遵循文末"新标准"。
>
> **新标准（原子扫描器隔离原则）**：以后所有原子扫描器（`WebScanner`/`ApiScanner`/...）之间不共享抓取、解析等内部实现模块，各自的能力在各自的包内自行实现。若确有明显重复的通用逻辑，优先考虑抽成无状态的纯函数工具库（放在 `internal/pkg` 一类的位置），而不是让某个扫描器的内部包（如 `crawler`）反过来成为其他扫描器的公共依赖。
