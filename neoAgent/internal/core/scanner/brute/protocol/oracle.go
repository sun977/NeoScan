...
func NewOracleCracker() *OracleCracker {
	// 常见的 Oracle SID 列表 (参考自 qscan/hydra)
	commonSIDs := []string{
...
func (c *OracleCracker) Name() string {
	return "oracle"
}

func (c *OracleCracker) Mode() brute.AuthMode {
	return brute.AuthModeUserPass
}

func (c *OracleCracker) Check(ctx context.Context, host string, port int, auth brute.Auth) (bool, error) {
	// 1. 获取 SID
	// 优先使用 auth.Other 中指定的 SID (假设 Auth 结构体支持扩展字段)
	// 如果没有指定，则尝试探测
	sid := "orcl" // 默认值
	if val, ok := auth.Other["sid"]; ok && val != "" {
		sid = val
	} else {
		// 探测 SID (仅在第一次未指定时尝试)
		// 注意：这种简单的探测方式在每次 Check 时都会执行，效率较低
		// 理想情况下，应该在 BruteScanner 层面先进行服务识别和 SID 探测，然后将 SID 传入
		// 这里为了兼容性，我们尝试探测前几个最常用的 SID
		detectedSID := c.probeSID(ctx, host, port)
		if detectedSID != "" {
			sid = detectedSID
		}
	}
	...
	// 尝试爆破
	return c.checkWithSID(ctx, host, port, sid, auth)
}

// probeSID 尝试探测 SID
func (c *OracleCracker) probeSID(ctx context.Context, host string, port int) string {
	// 仅尝试 Top 5 SID 以避免超时
	topSIDs := []string{"orcl", "xe", "ORACLE", "ora11g", "ora10g"}

	// 如果配置了 commonSIDs 且不为空，也可以考虑遍历更多，但要注意耗时
	// 这里为了性能，只尝试最常见的

	for _, sid := range topSIDs {
		// 尝试连接，使用一个不存在的用户，检查是否返回 ORA-01017 (无效用户名/密码)
		// 如果返回 ORA-12505 (SID 无效)，则说明 SID 错误
		// 如果返回 ORA-01017，说明 SID 正确

		auth := brute.Auth{Username: "neo_probe_user", Password: "neo_probe_pass"}
		_, err := c.checkWithSID(ctx, host, port, sid, auth)

		if err == nil {
			// 竟然登录成功了？那肯定也是 SID 正确
			return sid
		}

		errMsg := err.Error()
		if strings.Contains(errMsg, "ORA-01017") || strings.Contains(errMsg, "logon denied") {
			// 鉴权失败，说明 SID 是存在的！
			return sid
		}
	}
...
func (c *OracleCracker) checkWithSID(ctx context.Context, host string, port int, sid string, auth brute.Auth) (bool, error) {
	// 构建连接字符串
	// oracle://user:pass@host:port/sid
	connURL := fmt.Sprintf("oracle://%s:%s@%s:%d/%s",
		url.QueryEscape(auth.Username),
		url.QueryEscape(auth.Password),
		host, port, sid)
...
// handleError 解析 Oracle 错误
func (c *OracleCracker) handleError(err error) (bool, error) {
	if err == nil {
		return true, nil
	}
	...
	// 密码过期或需要修改，也算爆破成功（至少密码是对的）
	// ORA-28001: the password has expired
	// ORA-28009: connection to sys should be as sysdba or sysoper
	if strings.Contains(errMsg, "ORA-28001") ||
		strings.Contains(errMsg, "ORA-28009") {
		// 虽然连接失败，但证明密码是正确的
		// 返回 true, nil 表示爆破成功
		return true, nil
	}
	...
	// connection refused
	// i/o timeout
	if strings.Contains(errMsg, "ORA-12505") ||
		strings.Contains(errMsg, "ORA-12514") ||
		strings.Contains(errMsg, "ORA-12170") ||
	...
	// 其他 ORA 错误，通常意味着连接成功但协议交互出错，倾向于认为密码错误或环境问题
	// 但为了稳妥，如果是 ORA 开头的其他错误，记录但不认为是连接失败
	if strings.Contains(errMsg, "ORA-") {
		return false, nil
	}
