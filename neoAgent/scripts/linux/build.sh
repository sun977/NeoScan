#!/bin/bash
#
# NeoScan-Agent Linux 平台构建脚本
#
# 用法说明：
#   ./build.sh [选项]
#
# 选项：
#   -o, --os <操作系统>    目标操作系统（windows, linux, darwin），默认：linux
#   -a, --arch <架构>      目标架构（amd64, arm64），默认：amd64
#   -v, --version <版本号> 指定版本号，如果不指定则从 version.go 读取
#   -h, --help              显示帮助信息
#
# 示例：
#   ./build.sh                          编译当前平台（Linux amd64）
#   ./build.sh -o windows -a amd64      编译 Windows amd64 版本
#   ./build.sh -o darwin -a arm64      编译 macOS ARM64 版本（Apple Silicon）
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "选项："
            echo "  -o, --os <操作系统>    目标操作系统（windows, linux, darwin），默认：linux"
            echo "  -a, --arch <架构>      目标架构（amd64, arm64），默认：amd64"
            echo "  -v, --version <版本号> 指定版本号"
            echo "  -h, --help              显示帮助信息"
            echo ""
            echo "示例："
            echo "  $0                          编译当前平台（Linux amd64）"
            echo "  $0 -o windows -a amd64      编译 Windows amd64 版本"
            echo "  $0 -o darwin -a arm64      编译 macOS ARM64 版本"
