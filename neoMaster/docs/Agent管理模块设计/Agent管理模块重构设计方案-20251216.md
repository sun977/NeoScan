...
**运行时同步 (Runtime Sync - 优化版)**:
- **懒加载/按需创建 (Lazy Creation)**: 
  - 当 Agent 注册上报一个新的 `TaskSupport` (e.g. "CloudScan") 时，如果在 `ScanType` 表中找不到，可以配置为**自动创建**一个占位 ScanType 和对应的 Tag。
  - 优点：无需在 ScanType CRUD 接口中埋点，解耦了管理后台与标签系统。
  - 缺点：可能产生垃圾数据（Agent 乱报数据）。
- **建议策略**: 保持 "Hook" 机制作为主同步手段，辅以 "启动时全量同步" 作为兜底。当管理员在后台新增 `ScanType` 时，同步创建 Tag。
...
**1. 注册时上报 (Register Time)**
   ...
   - Master 处理:
     ...
     - 调用 `tagService.SyncEntityTags(..., "agent_capability")` 将能力映射为系统标签。
