...
| **M2** | **结构映射器 (Converter)** | 实现从标准 `StageResult.Attributes` JSON 到 `Asset` 实体的映射逻辑。 | ⚪ 未开始 | `converter.go` 能将 `{"ports": [...]}` 转换为 `[]*AssetPort`。 |
...
#### 2.1 Mapper (结构映射器)
*   **文件**: `mapper.go`
*   **定位**: **Mapper (映射器)** 而非 Parser (解析器)。Agent 端已完成原始输出到标准 JSON 的归一化。
*   **核心设计**: 引入 `AssetBundle` 作为统一输出包，包含 `Host`, `Services`, `Webs`, `Vulns` 等所有资产实体。
*   **任务**:
    *   实现 `MapToAssetBundle(result)`: 统一入口，基于 `ResultType` 分发。
    *   实现 `mapPortScan(result)`: 映射端口扫描结果 -> `Host`, `[]Service`。
    *   实现 `mapWebEndpoint(result)`: 映射 Web 扫描结果 -> `Host`, `Web`, `WebDetail`。
    *   实现 `mapVulnFinding(result)`: 映射漏洞结果 -> `Vuln`。
*   **输入示例**:
    ```json
