...
| | **Mapper** | 🟡 **部分完成** | `MapToAssetBundle` 分发框架已完成，Web/Vuln/PoC 等核心映射可用；但 `ip_alive/password_audit/proxy_detection/directory_scan/subdomain_discovery/api_discovery/file_discovery/other_scan` 等映射仍为 TODO，**暂时忽略**（作为未来扫描类型扩展预留），当前会被显式报错并跳过（可观测）。 |
