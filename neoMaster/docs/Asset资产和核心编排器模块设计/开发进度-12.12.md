...
基于重构方案与代码审计，当前控制面（Control Plane）的整体完成度约 **92%**：Orchestrator 已实现真正的 **DAG 并行执行**（基于 `Predecessors` 依赖解析），并且调度链路已具备 **目标解析 -> 策略拦截 -> 任务生成 -> Pull 分发 -> CAS 认领** 的完整闭环。
...
- 多个 ResultType 的 Mapper 仍是 TODO，但已改为“显式错误 + 日志告警”，对应结果不会再静默丢弃（当前策略：Processor 记录错误后跳过该条结果）。
...
| | **Mapper** | 🟡 **部分完成** | `MapToAssetBundle` 分发框架已完成，Web/Vuln/PoC/IPAlive/PasswordAudit 等核心映射可用；但 `proxy_detection/directory_scan/subdomain_discovery/api_discovery/file_discovery/other_scan` 等映射仍为 TODO，**暂时忽略**（作为未来扫描类型扩展预留），当前会被显式报错并跳过（可观测）。 |
| | **Fingerprint Integration** | 🔄 **已移出** | 根据最新架构，指纹再识别逻辑已移出 ETL 热路径，转为离线治理任务。 |
| | **Merger** | 🟡 **部分完成** | Host/Service/Web(WebAssets)/WebDetail 的 Upsert 与 `syncToUnified` 主链路已完成，且已实现 Vuln 落库并建立 Host/Service/Web 的关联，Vuln 幂等性已升级为数据库唯一键约束 + Upsert；仍有待补齐：Host Tags/SourceStageIDs 合并、入库失败的错误策略（重试/死信/告警）。 |
| | **WebCrawlerDataHandler** | ✅ **已完成** | `Handle` 方法已实现，支持 Base64 截图解码存储 (可配置路径)、HTML Hash 计算与去重。 |
| **2.4 Asset Enrichment** | **FingerprintMatcher** | ✅ **已完成** | **架构重构**: 原 Offline Governance 已迁移至此。负责存量资产的指纹再识别。已实现基于 `AssetService` 的批量指纹匹配，并支持可选的 TagSystem 联动 (AutoTag) 逻辑。 |
...
| | **Rule Snapshot Source** | ✅ **已完成** | 实现了智能路径映射：优先读取配置，未配置则自动映射到 `rules/{type}`。支持热扩展新规则类型。 |

### 2. 下一步开发建议

根据 "重构路线图 (Implementation Path)"，目前系统处于 **Phase 5 的落地期**：调度与策略链路已基本就绪，且 **Master-Agent 规则同步机制已完成重构**；剩余的工作重点转向两类“真实缺口”——**数据闭环落库** 与 **跨端一致性/工程化**。

**推荐核心任务：先消灭“静默丢弃”，再推进跨端一致性与工程化**

**具体开发任务清单：**

1.  **Asset ETL Engine (补齐与加固) - P0**:
    *   **Data Integrity (数据完整性)**: 修复 `Host Tags` 与 `SourceStageIDs` 在合并时的覆盖问题，实现 JSON 数组的合并去重逻辑，确保资产画像和溯源的完整性。**(🔥 Next Action)**
    *   **Mapper 覆盖率**: *暂时忽略*。`proxy_detection` 等作为未来扩展项。
    *   **错误策略**: 引入死信队列/错误表，明确重试策略与告警触发条件。

2.  **Master-Agent 指纹同步机制 - P1**:
    *   **Secure Export**: 基于已注入的密钥，实现 Agent 专用的加密/签名导出接口，确保传输安全。
    *   **Heartbeat Integration**: ✅ **已完成**。
    *   **规则来源一致性**: ✅ **已完成**。

3.  **Asset Enrichment (资产丰富化 - 离线治理) - P2**:
    *   **FingerprintMatcher Job**: 实现后台任务 (Cron Job)，定期扫描 `AssetService` 中 `Product` 为空但 `Banner` 不为空的记录，调用指纹服务进行补全。
    *   **Tag Linkage**: ✅ **Logic Completed**。逻辑已集成在 `FingerprintMatcher` 中，等待 Job 触发。

4.  **Tool Adapter Layer (工具适配) - P3**:
    *   **Command Factory**: 实现 Nmap/Masscan 等工具的命令模板生成。
    *   **Output Normalizer**: 确保 Agent 上报的 JSON 格式与 Master 的 Mapper 兼容。

**决策**: 暂时搁置 Mapper 扩展，聚焦于核心数据链路的健壮性。下一步将优先解决 **ETL 层的 Host Tags/SourceStageIDs 合并逻辑缺失** 问题，防止资产画像和溯源数据丢失；随后推进 **ETL 错误处理策略** 和 **指纹库安全导出**。
