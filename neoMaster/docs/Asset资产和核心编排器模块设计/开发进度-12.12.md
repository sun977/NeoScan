...
1.  **Asset ETL Engine (补齐与加固) - P0**:
    *   **Data Integrity (数据完整性)**: ✅ **已完成**。修复了 Host SourceStageIDs 合并逻辑。
    *   **Mapper 覆盖率**: *暂时忽略*。`proxy_detection` 等作为未来扩展项。
    *   **错误策略**: ✅ **已完成核心逻辑**。实现了指数退避重试与错误分类（瞬时/持久），持久错误自动降级为死信 (AssetETLError)。待办：死信重放接口。
...
**决策**: 暂时搁置 Mapper 扩展，聚焦于核心数据链路的健壮性。下一步将优先解决 **ETL 错误处理策略 (死信队列)** 问题，防止异常数据丢失且无法追溯；随后推进 **指纹库安全导出** 和 **Enrichment Job**。
