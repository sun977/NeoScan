...
**推荐核心任务：完成 Master-Agent 指纹同步机制与漏洞映射**

**具体开发任务清单：**

1.  **Asset ETL Engine (查漏补缺)**:
    *   **Mapper 扩展**: 实现 `mapVulnFinding` (漏洞扫描结果) 的映射逻辑，确保漏洞数据能入库关联到 AssetVuln 表。
    *   **Mapper 扩展**: 实现 `mapPocScan` (PoC验证结果) 的映射逻辑。
...
**决策**: 优先完成 **mapVulnFinding** 以打通漏洞数据流，随后实现 **指纹同步 API** 以支持 Agent 的离线识别能力。
