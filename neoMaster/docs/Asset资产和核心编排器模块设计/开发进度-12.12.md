...
| | **Mapper** | 🟡 **部分完成** | `MapToAssetBundle` 分发框架已完成，Web/Vuln/PoC 等核心映射可用；但 `ip_alive/password_audit/proxy_detection/directory_scan/subdomain_discovery/api_discovery/file_discovery/other_scan` 等映射仍为 TODO，当前会被显式报错并跳过（可观测，但仍然是数据缺口）。 |
...
1.  **Asset ETL Engine (补齐与加固) - P0**:
    *   **Mapper 覆盖率**: 补齐 `proxy_detection`, `directory_scan`, `subdomain_discovery` 等缺失的映射逻辑。短期底线是“显式错误 + 可观测告警”，长期要把错误落到可查询的错误表/死信队列，别指望在日志里捞。**(🚧 进行中)**
...
2.  **Master-Agent 指纹同步机制 - P1**:
    ...
    *   **规则来源一致性**: ✅ **已完成**。实现了 `Publish` 机制 (DB -> Disk)，支持规则的启用/禁用 (`Enabled` 字段)，仅发布启用规则，并自动更新文件 mtime 触发版本变更。

3.  **Asset Enrichment (资产丰富化 - 离线治理) - P2**:
    *   **FingerprintMatcher Job**: 实现后台任务，定期扫描 `AssetService` 中 `Product` 为空但 `Banner` 不为空的记录，调用指纹服务进行补全。
    *   **Tag Linkage**: 实现指纹识别结果自动打标签 (TagSystem) 的逻辑，支持配置是否启用此联动。
...
**决策**: 当前已完成 **规则同步服务的通用化重构**；下一步优先消灭 **ETL 层的静默丢弃** 并补齐 Mapper 覆盖率与错误策略；随后把 **指纹同步** 做到“跨端一致 + 可运维”（加密传输、心跳集成）；最后再推进 **工具适配层** 与 **资源调度优化**。
