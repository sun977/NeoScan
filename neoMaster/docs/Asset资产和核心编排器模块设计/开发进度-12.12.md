...
| | **Mapper** | ✅ **已完成** | `MapToAssetBundle` 已实现，支持 IP/Port/Service 等核心类型的映射逻辑。待扩展更多 ResultType。 |
...
| | **WebCrawlerDataHandler** | 🟡 **部分完成** | 接口已定义，Handle 框架已建立。**缺失**: Base64 截图解码与文件存储、HTML Hash 计算。 |

### 2. 下一步开发建议

根据 "重构路线图 (Implementation Path)"，目前系统处于 **Phase 4: The Cleaner (数据清洗与闭环)** 的攻坚阶段。

**推荐核心任务：完成 Web 数据处理与 Mapper 扩展**

**具体开发任务清单：**

1.  **Asset ETL Engine (收尾)**:
    *   **WebCrawlerDataHandler 实现**: 填充 `Handle` 方法，解码 Base64 截图并保存到本地或对象存储；计算 HTML Hash。
    *   **Mapper 扩展**: 实现 `mapWebEndpoint`，将 Web 扫描结果映射为 `AssetBundle`，确保 Web 资产能被正确入库。
    *   **单元测试**: 为 `Merger` 和 `Processor` 编写单元测试，验证 Upsert 逻辑和指纹策略。
...
**决策**: 优先完成 **WebCrawlerDataHandler** 和 **mapWebEndpoint**，打通 Web 资产的入库流程，从而完成整个 Data Plane 的闭环。
