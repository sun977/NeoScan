...
数据面（Data Plane）方面："左半边"（目标生成）已完成；"右半边"（结果回流与清洗）已具备 **摄入 + 队列 + Worker 框架 + Host/Service/Web/Unified 入库主链路**，但 ETL 的映射覆盖率与合并细节仍有缺口：

- 多个 ResultType 的 Mapper 仍是 TODO，且当前实现会导致对应结果被静默丢弃（返回 `nil, nil` -> Merger 直接跳过）。
- Vuln 的合并/落库仍是 TODO。
- WebDetail 仍存在关联缺口（依赖 WebID，缺失时会被跳过）。
...
**推荐核心任务：先把“数据闭环真实落库”补齐，再把“指纹同步工程化”做完整**
...
**决策**: 优先补齐 **ETL 的真实落库链路**（尤其是 Vuln 与 WebDetail 关联）并消灭“静默丢弃”这类坏味道；随后把 **指纹同步** 做到“跨端一致 + 可运维”（心跳集成、缓存、版本触发）；最后再推进 **工具适配层** 与 **资源调度优化**。
