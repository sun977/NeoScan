...
### 2.4 Offline Governance (离线治理 - 独立后台任务)
*   **职责**: 对已入库的资产数据进行持续的清洗、标准化和再识别，不阻塞实时 ETL 流程。
*   **核心组件**:
    *   **FingerprintMatcher (Standardizer & Re-identifier)**: 存量资产指纹再识别。
        *   **机制**: 支持灵活配置规则，从 `AssetUnified` 宽表（或各实体表）中提取关键特征字段（如 Banner, Title, Headers, Body），批量调用指纹服务进行二次匹配与清洗。
        *   **触发**: 指纹库更新时触发，或定时 Cron Job。
        *   **功能拆解**:
            1.  **指纹二次匹配 (Re-identification)**:
                *   提取字段 -> 匹配指纹规则 -> 更新 `Product`, `Version`, `CPE` 等基础属性。
                *   此过程**不**强制依赖打标系统，可独立运行。
            2.  **标签联动 (Tag Linkage)**:
                *   根据更新后的资产属性，调用 `TagService.AutoTag` 或 `TagService.SubmitPropagationTask` 自动应用打标规则。
                *   示例：匹配到 `Product=Shiro` -> 自动打上 `Tag:Component:Shiro`。
        *   **执行模式**:
            *   **场景 1 (Pipeline)**: 指纹匹配 -> 成功更新属性 -> 触发打标规则 (可选)。
            *   **场景 2 (Clean Only)**: 仅进行指纹清洗和属性补全，不涉及标签变动。
        *   **价值**: 实现“不发包发现新漏洞”的能力，并维护资产画像的持续鲜活。
...
4.  **Phase 4: The Cleaner (主链路已打通，工程化未完)**
    *   实现 `Result Ingestor` 与 `Asset ETL Engine` 主链路。
    *   打通 "扫描 -> 结果 -> 资产" 的闭环（Host/Service/Web/WebDetail/Unified/Vuln）。
    *   未完成：Mapper 覆盖率、入库失败策略（重试/死信/告警）、Vuln 幂等性的数据库唯一约束。
