...
5.  **规则扩散与回溯 (Rule Propagation & Backfill)**:
    *   **通用场景**: 当创建或修改一条打标规则时（例如网段规则、漏洞名规则），不仅影响未来入库的数据，还应能自动"回溯"并标记系统中已存在的历史数据。
    *   **典型案例**: 网段继承（给网段打标 -> 扩散到网段下所有 IP，主域名打标 -> 扩散到主域名下所有子域名）。
...
## 验收标准
### 功能验收
- [x] **独立性**: 标签模块无任何业务实体的硬编码引用（使用 Interface{} 或 Generic string ID）。
- [x] **规则引擎**: 支持配置 JSON 格式的复杂规则。
- [x] **调用灵活**: ETL 流程只需调用 `TagService.AutoTag(entity)` 即可，无需关心内部用了哪个 Tagger。
- [x] **树操作**: 支持创建、删除、重命名节点，支持路径查询。
  - *注*: 移动节点 (Update Parent) 目前受限，需使用未来的 MoveTag 接口以保证路径一致性。
- [x] **规则扩散**: 提供通用的 `PropagateRule` 接口，支持对任一规则触发历史数据回溯打标。

### 质量验收
- [x] **性能**: 自动打标过程不应阻塞主业务流程（建议异步或高性能同步）。
  - *实现*: 使用内存缓存 + 预解析规则，匹配性能极高 (O(M))。
- [x] **扩展性**: 新增一种打标规则类型不需要修改数据库结构。
