...
// @param rule 扫描规则对象
// @return 错误信息
func (r *ScanRuleRepository) CreateScanRule(ctx context.Context, rule *orchestrator_model_drop.ScanRule) error {
...
// @param id 扫描规则ID
// @return 扫描规则对象和错误信息
func (r *ScanRuleRepository) GetScanRuleByID(ctx context.Context, id uint) (*orchestrator_model_drop.ScanRule, error) {
	var rule orchestrator_model_drop.ScanRule
...
// @param name 规则名称
// @return 扫描规则对象和错误信息
func (r *ScanRuleRepository) GetScanRuleByName(ctx context.Context, name string) (*orchestrator_model_drop.ScanRule, error) {
	var rule orchestrator_model_drop.ScanRule
...
// @param rule 扫描规则对象
// @return 错误信息
func (r *ScanRuleRepository) UpdateScanRule(ctx context.Context, rule *orchestrator_model_drop.ScanRule) error {
	rule.UpdatedAt = time.Now()
...
func (r *ScanRuleRepository) DeleteScanRule(ctx context.Context, id uint) error {
	err := r.db.WithContext(ctx).Delete(&orchestrator_model_drop.ScanRule{}, id).Error
	if err != nil {
		// 记录删除失败日志
...
func (r *ScanRuleRepository) GetScanRuleList(ctx context.Context, offset, limit int, ruleType *orchestrator_model_drop.ScanRuleType, severity *orchestrator_model_drop.ScanRuleSeverity, status *orchestrator_model_drop.ScanRuleStatus) ([]*orchestrator_model_drop.ScanRule, int64, error) {
	var rules []*orchestrator_model_drop.ScanRule
	var total int64

	query := r.db.WithContext(ctx).Model(&orchestrator_model_drop.ScanRule{})
...
// @param ruleType 规则类型
// @return 扫描规则列表和错误信息
func (r *ScanRuleRepository) GetScanRulesByType(ctx context.Context, ruleType orchestrator_model_drop.ScanRuleType) ([]*orchestrator_model_drop.ScanRule, error) {
	var rules []*orchestrator_model_drop.ScanRule
	err := r.db.WithContext(ctx).Where("type = ? AND status = ?", ruleType, orchestrator_model_drop.ScanRuleStatusEnabled).Find(&rules).Error
	if err != nil {
		logger.LogError(err, "", 0, "", "scan_rule_list", "GET", map[string]interface{}{
...
// @param severity 严重程度
// @return 扫描规则列表和错误信息
func (r *ScanRuleRepository) GetScanRulesBySeverity(ctx context.Context, severity orchestrator_model_drop.ScanRuleSeverity) ([]*orchestrator_model_drop.ScanRule, error) {
	var rules []*orchestrator_model_drop.ScanRule
	err := r.db.WithContext(ctx).Where("severity = ? AND status = ?", severity, orchestrator_model_drop.ScanRuleStatusEnabled).Find(&rules).Error
	if err != nil {
		logger.LogError(err, "", 0, "", "scan_rule_list", "GET", map[string]interface{}{
...
// @param ruleType 规则类型过滤（可选）
// @return 扫描规则列表和错误信息
func (r *ScanRuleRepository) GetActiveRules(ctx context.Context, ruleType *orchestrator_model_drop.ScanRuleType) ([]*orchestrator_model_drop.ScanRule, error) {
	var rules []*orchestrator_model_drop.ScanRule
	query := r.db.WithContext(ctx).Where("status = ?", orchestrator_model_drop.ScanRuleStatusEnabled)
...
func (r *ScanRuleRepository) ScanRuleExists(ctx context.Context, name string) (bool, error) {
	var count int64
	err := r.db.WithContext(ctx).Model(&orchestrator_model_drop.ScanRule{}).Where("name = ?", name).Count(&count).Error
	if err != nil {
...
// @return 扫描规则列表和错误信息
func (r *ScanRuleRepository) GetScanRulesByScope(ctx context.Context, scope string) ([]*orchestrator_model_drop.ScanRule, error) {
	var rules []*orchestrator_model_drop.ScanRule
	err := r.db.WithContext(ctx).Where("JSON_CONTAINS(applicable_scope, ?) AND status = ?", scope, orchestrator_model_drop.ScanRuleStatusEnabled).Find(&rules).Error
	if err != nil {
		logger.LogError(err, "", 0, "", "scan_rule_list", "GET", map[string]interface{}{
...
// @param status 新状态
// @return 错误信息
func (r *ScanRuleRepository) UpdateScanRuleStatus(ctx context.Context, id uint, status orchestrator_model_drop.ScanRuleStatus) error {
	err := r.db.WithContext(ctx).Model(&orchestrator_model_drop.ScanRule{}).Where("id = ?", id).Updates(map[string]interface{}{
		"status":     status,
		"updated_at": time.Now(),
	}).Error
	if err != nil {
...
func (r *ScanRuleRepository) EnableScanRule(ctx context.Context, id uint) error {
	return r.UpdateScanRuleStatus(ctx, id, orchestrator_model_drop.ScanRuleStatusEnabled)
}
...
func (r *ScanRuleRepository) DisableScanRule(ctx context.Context, id uint) error {
	return r.UpdateScanRuleStatus(ctx, id, orchestrator_model_drop.ScanRuleStatusDisabled)
}
...
// @param rule 扫描规则对象
// @return 错误信息
func (r *ScanRuleRepository) UpdateScanRuleWithTx(ctx context.Context, tx *gorm.DB, rule *orchestrator_model_drop.ScanRule) error {
	rule.UpdatedAt = time.Now()
...
func (r *ScanRuleRepository) DeleteScanRuleWithTx(ctx context.Context, tx *gorm.DB, id uint) error {
	err := tx.WithContext(ctx).Delete(&orchestrator_model_drop.ScanRule{}, id).Error
	if err != nil {
		// 记录删除失败日志
...
func (r *ScanRuleRepository) UpdateScanRuleFields(ctx context.Context, id uint, fields map[string]interface{}) error {
	...
	err := r.db.WithContext(ctx).Model(&orchestrator_model_drop.ScanRule{}).Where("id = ?", id).Updates(fields).Error
	if err != nil {
		logger.LogError(err, "", id, "", "scan_rule_update", "PUT", map[string]interface{}{
