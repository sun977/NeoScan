...
func (h *ProjectConfigHandler) CreateProjectConfig(c *gin.Context) {
	...
	// 解析请求体
	var req orchestrator_model_drop.ProjectConfig
...
func (h *ProjectConfigHandler) ListProjectConfigs(c *gin.Context) {
	...
	// 解析状态过滤参数
	var statusFilter *orchestrator_model_drop.ProjectConfigStatus
	if status != "" {
		var s orchestrator_model_drop.ProjectConfigStatus
		switch status {
		case "inactive", "0":
			s = orchestrator_model_drop.ProjectConfigStatusInactive
		case "active", "1":
			s = orchestrator_model_drop.ProjectConfigStatusActive
		case "archived", "2":
			s = orchestrator_model_drop.ProjectConfigStatusArchived
		default:
			// 无效状态，忽略过滤
			statusFilter = nil
		}
...
func (h *ProjectConfigHandler) EnableProjectConfig(c *gin.Context) {
	h.updateProjectConfigStatus(c, orchestrator_model_drop.ProjectConfigStatusActive, "enable_project_config", "启用项目配置")
}
...
func (h *ProjectConfigHandler) DisableProjectConfig(c *gin.Context) {
	h.updateProjectConfigStatus(c, orchestrator_model_drop.ProjectConfigStatusInactive, "disable_project_config", "禁用项目配置")
}
...
// 私有方法：更新项目配置状态
func (h *ProjectConfigHandler) updateProjectConfigStatus(c *gin.Context, status orchestrator_model_drop.ProjectConfigStatus, operation, message string) {
	// 获取请求上下文信息
	clientIP := utils.GetClientIP(c)
	userAgent := c.GetHeader("User-Agent")
	requestID := c.GetHeader("X-Request-ID")
	...
	if status == orchestrator_model_drop.ProjectConfigStatusActive {
...
// 私有方法：验证项目配置请求参数
func (h *ProjectConfigHandler) validateProjectConfigRequest(req *orchestrator_model_drop.ProjectConfig) error {
	// 基础字段验证
