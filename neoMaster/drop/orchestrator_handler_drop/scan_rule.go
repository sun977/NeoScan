...
func (h *ScanRuleHandler) CreateScanRule(c *gin.Context) {
	...
	// 解析请求参数
	var req orchestrator_model_drop.CreateScanRuleRequest
	...
	// 转换请求结构体为实体结构体
	rule := &orchestrator_model_drop.ScanRule{
		Name:        req.Name,
		Description: req.Description,
		Type:        req.Type,
		Category:    req.Category,
		Severity:    orchestrator_model_drop.ScanRuleSeverity(req.Severity),
	...
	if req.Config != nil {
		if enabled, exists := req.Config["enabled"]; exists {
			if enabledBool, ok := enabled.(bool); ok && enabledBool {
				rule.Status = orchestrator_model_drop.ScanRuleStatusEnabled
			} else {
...
func (h *ScanRuleHandler) BatchImportScanRules(c *gin.Context) {
	...
	// 解析请求参数
	var req orchestrator_model_drop.ImportScanRulesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
...
func (h *ScanRuleHandler) BatchEnableScanRules(c *gin.Context) {
	...
	// 调用服务层批量启用规则
	result, err := h.scanRuleService.BatchUpdateScanRuleStatus(c.Request.Context(), req.RuleIDs, orchestrator_model_drop.ScanRuleStatusEnabled)
	if err != nil {
...
func (h *ScanRuleHandler) BatchDisableScanRules(c *gin.Context) {
	...
	// 调用服务层批量禁用规则
	result, err := h.scanRuleService.BatchUpdateScanRuleStatus(c.Request.Context(), req.RuleIDs, orchestrator_model_drop.ScanRuleStatusDisabled)
	if err != nil {
...
func (h *ScanRuleHandler) UpdateScanRule(c *gin.Context) {
	...
	// 解析请求体
	var req orchestrator_model_drop.UpdateScanRuleRequest
	...
	if req.Severity != nil {
		rule.Severity = orchestrator_model_drop.ScanRuleSeverity(*req.Severity)
	}
	if req.Priority != nil {
...
func (h *ScanRuleHandler) ListScanRules(c *gin.Context) {
	...
	// 解析查询参数
	var req orchestrator_model_drop.ListScanRulesRequest
	...
	var severity *orchestrator_model_drop.ScanRuleSeverity
	if req.Severity != nil {
		switch *req.Severity {
		case "low":
			s := orchestrator_model_drop.ScanRuleSeverityLow
			severity = &s
		case "medium":
			s := orchestrator_model_drop.ScanRuleSeverityMedium
			severity = &s
		case "high":
			s := orchestrator_model_drop.ScanRuleSeverityHigh
			severity = &s
		case "critical":
			s := orchestrator_model_drop.ScanRuleSeverityCritical
			severity = &s
		}
	}
...
func (h *ScanRuleHandler) MatchScanRules(c *gin.Context) {
	...
	// 解析请求体
	var req orchestrator_model_drop.MatchScanRulesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
...
func (h *ScanRuleHandler) ExportScanRules(c *gin.Context) {
	...
	// 解析查询参数
	var req orchestrator_model_drop.ExportScanRulesRequest
	if err := c.ShouldBindQuery(&req); err != nil {
...
func (h *ScanRuleHandler) TestScanRule(c *gin.Context) {
	...
	// 解析请求体
	var req orchestrator_model_drop.TestScanRuleRequest
	if err1 := c.ShouldBindJSON(&req); err1 != nil {
...
func (h *ScanRuleHandler) GetScanRulesByType(c *gin.Context) {
	...
	// 调用服务层按类型获取扫描规则
	rules, err := h.scanRuleService.GetScanRulesByType(c.Request.Context(), orchestrator_model_drop.ScanRuleType(ruleType))
	if err != nil {
...
func (h *ScanRuleHandler) GetScanRulesBySeverity(c *gin.Context) {
	...
	// 调用服务层按严重程度获取扫描规则
	rules, err := h.scanRuleService.GetScanRulesBySeverity(c.Request.Context(), orchestrator_model_drop.ScanRuleSeverity(severity))
	if err != nil {
...
// @param req 请求参数
// @return error 验证错误
func (h *ScanRuleHandler) validateScanRuleRequest(req *orchestrator_model_drop.CreateScanRuleRequest) error {
	// 验证规则名称
	if req.Name == "" {
