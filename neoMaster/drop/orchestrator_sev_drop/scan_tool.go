...
func (s *ScanToolService) CreateScanTool(ctx context.Context, tool *orchestrator_model_drop.ScanTool) (*orchestrator_model_drop.ScanTool, error) {
	// 参数验证 - Linus式：消除特殊情况
	if tool == nil {
...
func (s *ScanToolService) UpdateScanTool(ctx context.Context, id uint, tool *orchestrator_model_drop.ScanTool) (*orchestrator_model_drop.ScanTool, error) {
	// 参数验证
	if id == 0 {
...
func (s *ScanToolService) GetScanTool(ctx context.Context, id uint) (*orchestrator_model_drop.ScanTool, error) {
	// 参数验证
	if id == 0 {
...
func (s *ScanToolService) ListScanTools(ctx context.Context, offset, limit int, toolType *orchestrator_model_drop.ScanToolType, status *orchestrator_model_drop.ScanToolStatus) ([]*orchestrator_model_drop.ScanTool, int64, error) {
	// 参数验证
	if offset < 0 {
		offset = 0
	}
...
func (s *ScanToolService) DeleteScanTool(ctx context.Context, id uint) error {
	...
	// 检查工具是否正在使用
	if tool.Status == orchestrator_model_drop.ScanToolStatusTesting {
...
func (s *ScanToolService) EnableScanTool(ctx context.Context, id uint) error {
	return s.updateScanToolStatus(ctx, id, orchestrator_model_drop.ScanToolStatusEnabled, "enable_scan_tool")
}
...
func (s *ScanToolService) DisableScanTool(ctx context.Context, id uint) error {
	return s.updateScanToolStatus(ctx, id, orchestrator_model_drop.ScanToolStatusDisabled, "disable_scan_tool")
}
...
// @param tool 扫描工具配置对象
// @return 错误信息
func (s *ScanToolService) ValidateScanToolConfig(ctx context.Context, tool *orchestrator_model_drop.ScanTool) error {
	// 基础字段验证
...
func (s *ScanToolService) GetAvailableScanTools(ctx context.Context) ([]*orchestrator_model_drop.ScanTool, error) {
...
// @param toolType 工具类型
// @return 扫描工具列表和错误信息
func (s *ScanToolService) GetScanToolsByType(ctx context.Context, toolType orchestrator_model_drop.ScanToolType) ([]*orchestrator_model_drop.ScanTool, error) {
...
func (s *ScanToolService) InstallScanTool(ctx context.Context, id uint) error {
	...
	// 更新状态为已安装
	if err := s.scanToolRepo.UpdateScanToolStatus(ctx, id, orchestrator_model_drop.ScanToolStatusEnabled); err != nil {
...
func (s *ScanToolService) UninstallScanTool(ctx context.Context, id uint) error {
	...
	// 更新状态为未安装
	if err := s.scanToolRepo.UpdateScanToolStatus(ctx, id, orchestrator_model_drop.ScanToolStatusDisabled); err != nil {
		return fmt.Errorf("更新工具状态失败: %w", err)
	}
...
// 私有方法：更新扫描工具状态
func (s *ScanToolService) updateScanToolStatus(ctx context.Context, id uint, status orchestrator_model_drop.ScanToolStatus, operation string) error {
	// 参数验证
	if id == 0 {
...
// 私有方法：设置默认值
func (s *ScanToolService) setDefaultValues(tool *orchestrator_model_drop.ScanTool) {
	if tool.Status == 0 {
		tool.Status = orchestrator_model_drop.ScanToolStatusDisabled
	}

	if tool.Type == "" {
		tool.Type = orchestrator_model_drop.ScanToolTypeCustom
	}
...
func (s *ScanToolService) GetSystemToolStatus(ctx context.Context) (map[string]interface{}, error) {
	...
	for _, tool := range tools {
		...
		// 统计安装和启用状态
		if tool.Status == orchestrator_model_drop.ScanToolStatusEnabled || tool.Status == orchestrator_model_drop.ScanToolStatusTesting {
			installedCount++
		}
		if tool.Status == orchestrator_model_drop.ScanToolStatusEnabled {
			enabledCount++
		}
	}
