...
import (
	...
	"errors"
	"fmt"
	orchestrator_drop2 "neomaster/drop/orchestrator_drop"
	"strings"
	"time"
...
type WorkflowService struct {
	workflowRepo *orchestrator_drop2.WorkflowConfigRepository // 工作流配置仓库
	projectRepo  *orchestrator_drop2.ProjectConfigRepository  // 项目配置仓库
	scanToolRepo *orchestrator_drop2.ScanToolRepository       // 扫描工具仓库
	scanRuleRepo *orchestrator_drop2.ScanRuleRepository       // 扫描规则仓库
}

// NewWorkflowService 创建工作流服务实例
// 注入必要的Repository依赖，遵循依赖注入原则
func NewWorkflowService(
	workflowRepo *orchestrator_drop2.WorkflowConfigRepository,
	projectRepo *orchestrator_drop2.ProjectConfigRepository,
	scanToolRepo *orchestrator_drop2.ScanToolRepository,
	scanRuleRepo *orchestrator_drop2.ScanRuleRepository,
) *WorkflowService {
...
func (s *WorkflowService) CreateWorkflowConfig(ctx context.Context, config *orchestrator_model_drop.WorkflowConfig) (*orchestrator_model_drop.WorkflowConfig, error) {
	// 参数验证 - Linus式：消除特殊情况
	if config == nil {
...
func (s *WorkflowService) UpdateWorkflowConfig(ctx context.Context, id uint, config *orchestrator_model_drop.WorkflowConfig) (*orchestrator_model_drop.WorkflowConfig, error) {
	// 参数验证
	if id == 0 {
...
// @param id 工作流配置ID
// @return 工作流配置对象和错误信息
func (s *WorkflowService) GetWorkflowConfig(ctx context.Context, id uint) (*orchestrator_model_drop.WorkflowConfig, error) {
	// 参数验证
	if id == 0 {
...
func (s *WorkflowService) ListWorkflowConfigs(ctx context.Context, offset, limit int, status *orchestrator_model_drop.WorkflowStatus, projectID *uint, triggerType *orchestrator_model_drop.WorkflowTriggerType) ([]*orchestrator_model_drop.WorkflowConfig, int64, error) {
	// 参数验证
	if offset < 0 {
		offset = 0
	}
...
func (s *WorkflowService) DeleteWorkflowConfig(ctx context.Context, id uint) error {
	...
	// 检查工作流是否正在执行
	if config.Status == orchestrator_model_drop.WorkflowStatusActive {
...
// @param config 工作流配置对象
// @return 错误信息
func (s *WorkflowService) ValidateWorkflowConfig(ctx context.Context, config *orchestrator_model_drop.WorkflowConfig) error {
	// 基础字段验证
	...
	// 步骤定义验证
	if config.Steps != "" {
		var stepDefinition []orchestrator_model_drop.WorkflowStep
...
func (s *WorkflowService) EnableWorkflowConfig(ctx context.Context, id uint) error {
	return s.updateWorkflowConfigStatus(ctx, id, orchestrator_model_drop.WorkflowStatusActive, "enable_workflow_config")
}
...
func (s *WorkflowService) DisableWorkflowConfig(ctx context.Context, id uint) error {
	return s.updateWorkflowConfigStatus(ctx, id, orchestrator_model_drop.WorkflowStatusInactive, "disable_workflow_config")
}
...
// @return 工作流配置列表和错误信息
func (s *WorkflowService) GetWorkflowsByProject(ctx context.Context, projectID uint) ([]*orchestrator_model_drop.WorkflowConfig, error) {
	if projectID == 0 {
...
// 私有方法：更新工作流配置状态
func (s *WorkflowService) updateWorkflowConfigStatus(ctx context.Context, id uint, status orchestrator_model_drop.WorkflowStatus, operation string) error {
	// 参数验证
	if id == 0 {
...
// 私有方法：设置默认值
func (s *WorkflowService) setDefaultValues(config *orchestrator_model_drop.WorkflowConfig) {
	if config.Status == 0 {
		config.Status = orchestrator_model_drop.WorkflowStatusInactive
	}

	if config.TriggerType == "" {
		config.TriggerType = orchestrator_model_drop.WorkflowTriggerManual
	}
...
func (s *WorkflowService) GetSystemScanStatistics(ctx context.Context) (map[string]interface{}, error) {
	...
	for _, workflow := range workflows {
		...
		case orchestrator_model_drop.WorkflowStatusActive:
			activeCount++
		case orchestrator_model_drop.WorkflowStatusArchived:
			completedCount++
		case orchestrator_model_drop.WorkflowStatusInactive:
			failedCount++
		}
	}
