...
func (m *MiddlewareManager) GinJWTAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		logger.Info("Authentication request", map[string]interface{}{
			...
			"func_name":    "middleware.auth.GinJWTAuthMiddleware",
		})

		// 检查跳过路径(配置了路径就直接跳过认证)
		if m.isPathSkipped(requestPath, m.securityConfig.Auth.SkipPaths) {
			// 跳过认证专用用户 sysuser uid 2
			c.Set("user_id", uint(2))
			c.Set("username", "syswuser")
		...
		// 检查IP白名单(先判断是否启用白名单跳过功能)
		if m.securityConfig.Auth.EnableIPWhitelist && len(m.securityConfig.Auth.WhitelistIPs) > 0 {
			if utils.IsIPInWhitelist(clientIP, m.securityConfig.Auth.WhitelistIPs) {
				// 跳过认证专用用户 sysuser uid 2
				c.Set("user_id", uint(2))
				c.Set("username", "sysuser")
...
func (m *MiddlewareManager) isPathSkipped(requestPath string, skipPaths []string) bool {
