...
func (r *Router) setupOrchestratorRoutes(v1 *gin.RouterGroup) {
	...
	orchestrator.Use(r.middlewareManager.GinJWTAuthMiddleware())    // JWT认证中间件
	orchestrator.Use(r.middlewareManager.GinUserActiveMiddleware()) // 用户激活状态检查中间件
	{
		// ==================== 项目配置管理路由 ====================
		// 项目配置是扫描配置的核心 - 所有其他配置都依赖于项目
		projects := orchestrator.Group("/projects")
		{
			// 基础CRUD操作
		...
		// ==================== 扫描工具管理路由 ====================
		// 扫描工具是执行具体扫描任务的组件
		tools := orchestrator.Group("/tools")
		{
			// 基础CRUD操作
		...
		// ==================== 扫描规则管理路由 ====================
		// 扫描规则定义具体的扫描检查逻辑
		rules := orchestrator.Group("/rules")
		{
			// 基础CRUD操作
		...
		// ==================== 规则引擎管理路由 ====================
		// 规则引擎负责规则的执行、验证和管理
		ruleEngine := orchestrator.Group("/rule-engine")
		{
			// 规则执行相关
	...
	// ==================== 管理员专用扫描配置路由 ====================
	// 需要管理员权限的高级配置操作
	adminOrchestrator := v1.Group("/admin/orchestrator")
	adminOrchestrator.Use(r.middlewareManager.GinJWTAuthMiddleware())    // JWT认证中间件
	adminOrchestrator.Use(r.middlewareManager.GinUserActiveMiddleware()) // 用户激活状态检查中间件
	adminOrchestrator.Use(r.middlewareManager.GinAdminRoleMiddleware())  // 管理员权限检查中间件
	{
		// 系统级配置管理
		adminOrchestrator.GET("/system/config", r.projectConfigHandler.GetSystemScanConfig)    // 获取系统扫描配置
		adminOrchestrator.PUT("/system/config", r.projectConfigHandler.UpdateSystemScanConfig) // 更新系统扫描配置

		// 全局工具管理
		adminOrchestrator.POST("/tools/batch-install", r.scanToolHandler.BatchInstallScanTools)     // 批量安装扫描工具
		adminOrchestrator.POST("/tools/batch-uninstall", r.scanToolHandler.BatchUninstallScanTools) // 批量卸载扫描工具
		adminOrchestrator.GET("/tools/system-status", r.scanToolHandler.GetSystemToolStatus)        // 获取系统工具状态

		// 全局规则管理
		adminOrchestrator.POST("/rules/batch-import", r.scanRuleHandler.BatchImportScanRules)   // 批量导入扫描规则
		adminOrchestrator.POST("/rules/batch-enable", r.scanRuleHandler.BatchEnableScanRules)   // 批量启用扫描规则
		adminOrchestrator.POST("/rules/batch-disable", r.scanRuleHandler.BatchDisableScanRules) // 批量禁用扫描规则

		// 系统监控和统计
		adminOrchestrator.GET("/statistics", r.workflowHandler.GetSystemScanStatistics) // 获取系统扫描统计
		adminOrchestrator.GET("/performance", r.workflowHandler.GetSystemPerformance)   // 获取系统性能指标
	}
}
