...
- `types.go`
  ...
  - 目前包含：
    ...
    - `AgentModule`：Agent 管理域聚合（Manager/Monitor/Config/Task 服务与统一 Handler）。
    - `OrchestratorModule`：扫描编排器聚合（项目配置/工作流/扫描工具/扫描规则/规则引擎处理器与服务）。
...
- `agent.go`
  - 构建“Agent 管理域”模块的初始化函数：`BuildAgentModule`。
  - 典型装配流程：
    1) 初始化仓库：AgentRepository（gorm/MySQL）。
    2) 初始化服务：AgentManagerService、AgentMonitorService、AgentConfigService。
    3) **依赖注入**：接收 `OrchestratorModule` 传入的 `TaskDispatcher`，用于初始化 AgentTaskService。
    4) 初始化服务：AgentTaskService（依赖 TaskDispatcher）。
    5) 初始化处理器：AgentHandler（内部组合上述服务）。
    6) 组装并返回 `AgentModule`。
  - 注意：Agent 模块依赖编排器的任务分发组件，因此必须在 Orchestrator 模块之后初始化。

- `orchestrator.go`
  - 构建“扫描编排器”模块的初始化函数：`BuildOrchestratorModule`。
  - 典型装配流程：
    1) 初始化仓库：ProjectRepository、WorkflowRepository、ScanStageRepository、ScanToolTemplateRepository（均基于 gorm/MySQL）以及 TaskRepository（跨域引用）。
    2) **初始化核心组件**：PolicyEnforcer（策略执行器）、ResourceAllocator（资源调度器）、TaskDispatcher（任务分发器）。
    3) 初始化服务：ProjectService、WorkflowService、ScanStageService、ScanToolTemplateService。
    4) 初始化处理器：ProjectHandler、WorkflowHandler、ScanStageHandler、ScanToolTemplateHandler。
    5) 组装并返回 `OrchestratorModule`（包含核心组件 TaskDispatcher 以供 Agent 模块使用）。
  - 注意：Orchestrator 模块作为核心控制平面，负责初始化全局调度组件。
...
- **初始化顺序调整**：路由层必须**先**调用 `setup.BuildOrchestratorModule` 初始化编排器模块，获取 `TaskDispatcher`。
- 路由层通过调用 `setup.BuildAgentModule`（传入 `TaskDispatcher`）获取 Agent 管理域的 `AgentHandler`，用于注册 Agent 管理相关路由。
