...
- `agent.go`
  ...
  - 典型装配流程：
    ...
    2) 初始化服务：AgentManagerService、AgentMonitorService、AgentConfigService、AgentTaskService。
    3) 初始化处理器：AgentHandler（内部组合上述服务）。
    4) 组装并返回 `AgentModule`。
  - 注意：分组能力已在 ManagerService 内部实现，Handler 聚合对外暴露统一入口，router 仅负责路由注册。

<!-- - `orchestrator.go` 【待重写】
  - 构建“扫描编排器”模块的初始化函数：`BuildOrchestratorModule`。
  - 典型装配流程：
    1) 初始化仓库：ProjectConfigRepository、WorkflowConfigRepository、ScanToolRepository、ScanRuleRepository（均基于 gorm/MySQL）。
    2) 初始化服务：ProjectConfigService、WorkflowService、ScanToolService、ScanRuleService。
    3) 初始化处理器：ProjectConfigHandler、WorkflowHandler、ScanToolHandler、ScanRuleHandler（注意：`NewScanRuleHandler` 接受非指针 `ScanRuleService`）、RuleEngineHandler（传入 `nil` 的规则引擎实例与 `ScanRuleService`）。
    4) 组装并返回 `OrchestratorModule`。
  - 注意：规则引擎目前通过 ScanRuleService 统一管理缓存与执行，不再需要独立的规则引擎实例；router 仅从模块中获取处理器进行路由注册。 -->
...
- 路由层通过调用 `setup.BuildAgentModule` 获取 Agent 管理域的 `AgentHandler`，用于注册 Agent 管理相关路由。
