...
type Config struct {
	...
	Monitoring MonitoringConfig `yaml:"monitoring" mapstructure:"monitoring"`   // 监控配置
...
type SecurityConfig struct {
	...
	CSRF      CSRFConfig      `yaml:"csrf" mapstructure:"csrf"`             // CSRF配置
}
...
type RateLimitConfig struct {
	...
	RequestsPerMinute int  `yaml:"requests_per_minute" mapstructure:"requests_per_minute"` // 每分钟请求数限制
	Burst             int  `yaml:"burst" mapstructure:"burst"`                             // 突发请求数
}

// CSRFConfig CSRF配置
type CSRFConfig struct {
	Enabled bool   `yaml:"enabled" mapstructure:"enabled"` // 是否启用CSRF保护
	Secret  string `yaml:"secret" mapstructure:"secret"`   // CSRF密钥
}
