...
// RuleTestResult 规则测试结果结构
type RuleTestResult struct {
	RuleID   uint64                 `json:"rule_id"`   // 规则ID
	RuleName string                 `json:"rule_name"` // 规则名称
	Success  bool                   `json:"success"`   // 测试是否成功
	Matched  bool                   `json:"matched"`   // 是否匹配
	Data     map[string]interface{} `json:"data"`      // 测试结果数据
	Message  string                 `json:"message"`   // 测试消息
	Duration time.Duration          `json:"duration"`  // 测试耗时
	TestedAt time.Time              `json:"tested_at"` // 测试时间
	Error    string                 `json:"error"`     // 错误信息
}
