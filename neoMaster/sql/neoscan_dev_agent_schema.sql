...
CREATE TABLE `agent_scan_types` (
    ...
    `is_active` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否激活',
    `is_system` tinyint(1) DEFAULT '0' COMMENT '是否为系统内置类型',
...
-- 默认扫描类型 (新增is_system字段)
INSERT INTO `agent_scan_types` (`name`, `display_name`, `description`, `category`, `is_active`, `is_system`, `config_template`, `tag_id`) VALUES
('ipAliveScan', 'IP探活扫描', 'IP探活阶段，探测网段内存活IP', 'network', 1, 1, '{"timeout": 30, "threads": 100, "ping_count": 3}', 0),
('fastPortScan', '快速端口扫描', '快速端口扫描，默认端口的快速扫描', 'network', 1, 1, '{"timeout": 30, "threads": 100, "ports": "22,80,443,3389,3306,5432"}', 0),
('fullPortScan', '全量端口扫描', '全量端口扫描，全端口扫描，会带有端口对应的服务信息', 'network', 1, 1, '{"timeout": 60, "threads": 50, "ports": "1-65535", "service_detection": true}', 0),
('serviceScan', '服务扫描', '服务扫描，如果端口识别不携带服务识别，这一步单独做一次服务识别', 'service', 1, 1, '{"timeout": 120, "version_detection": true, "script_scan": true}', 0),
('vulnScan', '漏洞扫描', '漏洞扫描，发现安全漏洞', 'security', 1, 1, '{"severity": "medium", "timeout": 300, "plugins": ["cve", "exploit"], "update_db": true}', 0),
('pocScan', 'POC扫描', 'POC扫描，结合给定的POC工具或者脚本识别，属于高精度的vulnScan', 'security', 1, 1, '{"timeout": 600, "poc_templates": "/opt/poc-templates", "custom_scripts": true}', 0),
('webScan', 'Web扫描', 'Web扫描，识别出有web服务或者web框架cms等执行web扫描', 'web', 1, 1, '{"crawl_depth": 3, "timeout": 600, "check_sql_injection": true, "check_xss": true}', 0),
('passScan', '弱密码扫描', '弱密码扫描，识别出有密码的服务后探测默认/弱口令检查', 'security', 1, 1, '{"timeout": 300, "wordlist": "/opt/wordlists", "protocols": ["ssh", "ftp", "mysql", "mssql"]}', 0),
('proxyScan', '代理服务探测扫描', '代理服务探测扫描，识别出有代理服务后进行代理扫描', 'network', 1, 1, '{"timeout": 180, "proxy_types": ["http", "https", "socks4", "socks5"]}', 0),
('dirScan', '目录扫描', '目录扫描，识别出有web系统后对系统进行目录扫描', 'web', 1, 1, '{"timeout": 300, "wordlist": "/opt/dirbuster", "extensions": ["php", "asp", "jsp"]}', 0),
('subDomainScan', '子域名扫描', '子域名扫描，识别出有web系统后对系统进行子域名扫描', 'web', 1, 1, '{"timeout": 600, "dns_servers": ["8.8.8.8", "1.1.1.1"], "wordlist": "/opt/subdomains"}', 0),
('apiScan', 'API资产扫描', 'API资产扫描，对需要探测的系统所暴露的API进行API资产扫描', 'web', 1, 1, '{"timeout": 300, "swagger_detection": true, "graphql_detection": true}', 0),
...
INSERT INTO `agents` (`agent_id`, `hostname`, `ip_address`, `port`, `version`, `status`, `os`, `arch`, `cpu_cores`, `memory_total`, `disk_total`, `task_support`, `feature`, `token`, `token_expiry`, `last_heartbeat`, `remark`) VALUES
