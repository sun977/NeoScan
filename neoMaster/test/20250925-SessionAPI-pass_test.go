// SessionAPI测试文件
// 测试了会话管理API接口，包括获取活跃会话列表、撤销用户会话、权限不足测试和普通用户访问会话管理接口等
// 测试命令：go test -v -run TestSessionAPI ./test
...
func TestSessionAPI(t *testing.T) {
	RunWithTestEnvironment(t, func(ts *TestSuite) {
		...
		t.Run("会话管理接口", func(t *testing.T) {
			...
			_, err := ts.SessionService.Login(context.Background(), loginReq, "192.0.2.1", "test-user-agent")
...
func testSessionManagementAPI(t *testing.T, router *gin.Engine, ts *TestSuite) {
	...
	t.Run("撤销用户会话", func(t *testing.T) {
		req := httptest.NewRequest("POST", "/api/v1/admin/sessions/user/"+userID+"/revoke", nil)
		req.Header.Set("Authorization", "Bearer "+adminAccessToken)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
	...
	// 测试获取不存在用户的活跃会话列表
	t.Run("获取不存在用户的活跃会话列表", func(t *testing.T) {
