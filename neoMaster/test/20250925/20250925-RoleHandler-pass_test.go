...
func TestRoleHandler(t *testing.T) {
	RunWithTestEnvironment(t, func(ts *TestSuite) {
		...
		loginResp, err := ts.SessionService.Login(context.Background(), &system2.LoginRequest{Username: "roleadmin", Password: "password123"}, "127.0.0.1", "test-agent")
