// Package test 完整API接口测试
// 测试NeoScan Master v4.0的所有API接口
package test
...
// setupFullTestRouter 设置完整测试路由
func setupFullTestRouter(ts *TestSuite) *gin.Engine {
	router := gin.New()
...
func testAdminAPI(t *testing.T, router *gin.Engine, ts *TestSuite) {
	// 创建管理员用户
	adminUser := ts.CreateTestUser(t, "admintestuser", "admin@example.com", "password123")
