...
### 核心特性
- **分布式架构**: Master-Agent 架构，采用 Thin Agent 模式，Master 为中央大脑，Agent 为无状态执行器
- **多模块扫描**: 资产扫描、Web扫描、POC漏洞扫描、目录扫描、域名扫描、弱口令扫描、代理扫描
- **智能调度**: 基于Agent负载的智能任务分发和调度，Master 负责完整的状态流转
...
后端技术:
  - 语言: Go 1.19+
  - 框架: Gin (HTTP)
  - 数据库: MySQL 8.0+ (主数据), SQLite (Agent本地) --- 临时存储,有可能后续删除该项
  - 缓存: Redis 6.0+
  - 消息队列: RabbitMQ 3.8+
  - 认证: JWT + RBAC
...
### Agent节点功能
1. **执行模块 (Executor)**:
   - **扫描执行**: 接收原子任务，执行Nmap/Nuclei等工具
   - **结果封装**: 标准化输出，无状态反馈

2. **扫描模块 (Legacy/Plugins)**:
...
### HTTP/WebSocket服务 (Master-Agent)
Master 和 Agent 之间采用纯 HTTP 协议进行交互，Agent 仅作为被动执行器。

- **任务下发**: HTTP POST /api/agent/v1/tasks
- **结果回传**: HTTP POST /api/master/v1/results
- **心跳保活**: HTTP POST /api/master/v1/heartbeat (或 WebSocket)
- **实时控制**: WebSocket (用于即时停止任务等操作)
