...
type Plugin struct {
    ...
    Repository    string    `gorm:"size:500" json:"repo"`                 // 代码仓库
...
#### 4.1.1 Agent注册模型
**请求路径**: `POST /api/master/v1/agent/register`
...
#### 4.1.2 心跳模型
**请求路径**: `POST /api/master/v1/heartbeat`
...
type HeartbeatResponse struct {
    Success       bool           `json:"success"`
    Commands      []TaskCommand  `json:"commands"`       // 停止任务等指令
    ConfigUpdates []ConfigUpdate `json:"config_updates"` // 配置更新指令
}
```

#### 4.1.3 任务分发模型
**请求路径**: `POST /api/agent/v1/tasks`
...
type TaskDispatchRequest struct {
    ...
    Config   string `json:"config"` // JSON格式的任务配置
}
...
#### 4.1.4 任务结果上报模型
**请求路径**: `POST /api/master/v1/results`
